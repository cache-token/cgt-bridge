...
/// @title The CacheGold Leveraged Token Contract
/// @author Monty Python
contract CGT is ERC20, Ownable {
    ...
    /**
     * @dev Contructor for the CacheGold Leveraged Token
     * @param feeAddress The address where fees are collected
     */
    constructor(address feeAddress) ERC20("LeveragedCGT", "LCGT") {
     ...
     */
    modifier onlyCACHE() {
        require(router == _msgSender(), "Caller is not CACHE ADMIN");
        _;
    }
     ...
     */
    function setFeeEnforcer(address enforcer)
        external
        onlyOwner
        returns (bool)
    {
