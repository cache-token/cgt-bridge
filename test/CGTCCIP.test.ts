...
import { CacheGoldCCIP, CacheGold } from "../typechain";
...
let cgt: CacheGoldCCIP;
let randomContract = "";
// Start test block
describe("CGT complies with cache gold standards", () => {
  let owner: Wallet;
  let feeAddr: Wallet;
  ...
  const expectTotals = async (instance: CacheGoldCCIP) => {
  ...
  const fixture = async () => {
    const _cgt = await ethers.getContractFactory("CacheGoldCCIP");
    cgt = (await _cgt.deploy()) as CacheGoldCCIP;
    const _lockedGold = await ethers.getContractFactory("LockedGoldOracle");
    const lockedGold = await _lockedGold.deploy();
    randomContract = lockedGold.address;
  ...
  beforeEach("create fixture loader", async () => {
    [owner, feeAddr, external1, external2, external3, bridge, ccip] = await (
      ethers as any
    ).getSigners();
    ...
    cgt.setFeeExempt(feeAddr.address)
  });

  // Just make sure non-owners can't call protected functions
  it("Test onlyOwner protection", async function () {
    await expect(cgt.connect(external1).setFeeAddress(feeAddr.address)).to.be
      .reverted;
    await expect(cgt.connect(external1).setFeeExempt(feeAddr.address)).to.be
      .reverted;
    await expect(cgt.connect(external1).unsetFeeExempt(feeAddr.address)).to.be
      .reverted;
    await expect(
  ...
  it("Test onlybridge protection", async function () {
    await expect(cgt.connect(external1).setFxManager(feeAddr.address)).to.be
      .reverted;
  });
  ...
  it("Test storage and transfer fees on real looking transfers", async function () {
    ...
    // Exec transfer
    result = await cgt.connect(external3).transfer(ccip.address, TOKEN.mul(10));
    ...
    const redeemBalance = await cgt.balanceOfNoFees(ccip.address);
  ...
  it("Test changing transfer fee", async function () {
    ...
    let sendAllCalc = maximumTransferAmount(
      currentTransferFee.toNumber(),
      amount
    );
  ...
  it("Test maximumTransferAmount", async function () {
    ...
    await cgt.connect(external2).transfer(ccip.address, amount);
    const finalBalance = await cgt.balanceOfNoFees(external2.address);
    expect(finalBalance.toNumber() === 0);

    // Check zero address
    await expect(cgt.maximumTransferAmount(ethers.constants.AddressZero))
      .reverted;
    ...
    for (const day of [1, 365, 366, 730, 731, 1095]) {
      for (const tokens of [1, 50, 123, 1234, 12345, 123456]) {
        for (const basisPoints of [1, 3, 5, 7, 9]) {
          ...
          // Calcuate the amount needed to send entire balance to another address
          const calcSendAll = await cgt.maximumTransferAmount(
            external2.address
          );
          ...
          // Now try to actually send all balance
          await cgt.connect(external2).transfer(ccip.address, calcSendAll);
